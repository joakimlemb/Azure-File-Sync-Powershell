...
$i = 0
Get-ChildItem -File -Path "$path" -Recurse | Where-Object { ($_.LastWriteTime -gt (get-date).AddDays(-$days_old)) -and ([Disk.SizeInfo]::GetCompressedFileSizeFunction($_.FullName,$false) -lt 65536) } | % {
    $i++
    $_.FullName
    Invoke-StorageSyncFileRecall -Path $_.FullName
}
Write-Output "Recalled $i files"